...
GPRO.namespace('PhantichByProGroup');
GPRO.PhantichByProGroup = function () {
    var Global = {
        UrlAction: {
    ...
    var RegisterEvent = function () {
        $('[re-select-product-group]').click(function () {
            GetProductGroupSelect('select-product-type')
        });
        ...
        $('[re-select-product]').click(function () {
            GetProductSelectByProGroup($('#select-product-type').val(), 'select-product',false);
        });

        $('#select-product').change(function () {
            $('#tree-pt').empty();
            Global.Data.CommoAnaItems.length = 0;
            let _proId = $('#select-product').val();
            if (_proId && _proId != '0') {
                Global.Data.SelectedProductId = parseInt(_proId);
                FindCommo(parseInt(_proId), false);
            }
        });
    ...
    function DrawCommoAna(obj) {
        ...
        root.append(`<div class="div-lv1">
                        <div  ><i class="fa fa-shirtsinbulk"></i>${obj.Name}</div> 
                    </div>`);
        ...
        if (obj.Details && obj.Details.length > 0) {
            ...
            $.each(obj.Details, (key, _item) => {
                ...
                root.append(`<div class="div-lv2">
                                <div  ><i class="fa fa-industry"></i>${_item.Name}</div> 
                            </div>`);
    ...
    function BindData(Id, oType, wkId) {
        ...
        if (_filter) {
            ...
            //QTCN
            if (oType == 3) {
                $('#title-popup').html(`quy trình công nghệ: <span class ="red">${$('#select-product option:selected').text()}</span>`);
            ...
            if (oType == 6) {
                ...
                $('#title-popup').html(`phân tích cụm công đoạn : <span class ="red">${$('#select-product option:selected').text()}</span>`);
            ...
            if (oType == 8) {
                ...
                $('#title-popup').html(`thiết kế chuyền : <span class ="red">${$('#select-product option:selected').text()}</span>`);
                $('#jtable-tkc').show();
                ReloadList_TKC();
                $('[re_line_tkc]').click();
    ...
    function InitListTech_Cycle() {
        $('#' + Global.Element.JtableTech_Cycle).jtable({
            ...
            fields: {
                ...
                ti_le: {
                    title: 'Tỉ Lệ <br/><input disabled class="form-control" style="width:55px" value="100" type="text" id="tile-parent" onChange="Change()" /> %',
                    width: "5%",
                },
                Time_Percent: {
                    title: 'TGian Theo %',
                ...
                Description: {
                    ...
                    sorting: false,
                    display: function (data) {
                        txt = '<input type="text" disabled value="' + data.record.Description + '" des />';
                        return txt;
                    }
                }
            }
                        ...
                        if (result.Data.Id == 0) {
                            ...
                            if (result.Data.details.length > 0) {
                                ...
                                $.each(result.Data.details, function (index, item) {
                                    ...
                                    str += '<td><input disabled class="form-control" percent type="text" id="percent' + ii + '" value="100" onchange ="ResetTime_Percent(\'' + ii + '\')" /></td>';
                                    str += '<td>' + Math.round(((item.StandardTMU)) * 1000) / 1000 + '</td>';
                                    str += '<td>0</td>';
                                    str += '<td><input disabled class="form-control" description type="text"  value="' + item.Description + '"  /></td>';
                                    str += '</tr>';
                                    let _obj = {
                                        Id: 0,
                            ...
                            if (result.Data.details.length > 0) {
                                ...
                                $.each(result.Data.details, function (index, item) {
                                    ...
                                    str += '<td>' + Math.round((item.StandardTMU) * 1000) / 1000 + '</td>';
                                    str += '<td><input disabled class="form-control" percent type="text" id="percent' + ii + '" value="' + item.Percent + '" onchange ="ResetTime_Percent(\'' + ii + '\')" /></td>';
        ...
        $('#' + Global.Element.JtableManipulationArr).jtable({
            ...
            fields: {
                ...
                OrderIndex: {
                    ...
                    width: "5%",
                    display: function (data) {
                        var txt = $('<input disabled class="form-control text-center" stt type="text" value =\'' + data.record.OrderIndex + '\' />');
                ...
                ManipulationCode: {
                    ...
                    width: "15%",
                    display: function (data) {
                        var txt = $('<input disabled class="form-control" code_' + data.record.OrderIndex + ' list="manipulations" type="text" value="' + data.record.ManipulationCode + '" />');
                        txt.change(function () {
                ...
                Description: {
                    ...
                    width: "35%",
                    display: function (data) {
                        var txt = $('<input disabled class="form-control" des type="text" value="' + data.record.ManipulationName + '" />');
                        txt.change(function () {
                ...
                Loop: {
                    ...
                    width: "5%",
                    display: function (data) {
                        var txt = $('<input disabled class="form-control text-center" loop type="text" value="' + data.record.Loop + '"  onkeypress="return isNumberKey(event)"/>');
                        txt.change(function () {
