...
GPRO.Phantich4 = function () {
    var Global = {
        ...
        Element: {
            ...
            JtablePhaseLib: 'jtable-phase-lib',
            PopupPhaseLib: 'popup-phase-lib',
    ...
    //#region out events
    this.SetPhaseGroup = function (_objectId, _phasegroupId) {
        Global.Data.PhaseGroupId = _phasegroupId;
        Global.Data.ObjectId = _objectId;
    }
    ...
    function InitPhaseEvents() {
        ...
        $('#phase-index').change(function () {
            ...
            $('#phase-code').attr('code', ((Global.Data.PhaseAutoCode == null || Global.Data.PhaseAutoCode == '' ? '' : (Global.Data.PhaseAutoCode + '-')) + $('#phase-index').val()));
        });
        ...
        $('[cancel-create-phase]').click(function () {
            ...
            $('#phase-code').attr('code', ((Global.Data.PhaseAutoCode == null || Global.Data.PhaseAutoCode == '' ? '' : (Global.Data.PhaseAutoCode + '-')) + (Global.Data.phaseLastIndex + 1)));
                    ...
                    $.each(_item.Details, (key, item1) => {
                        ...
                        if (item1.Details && item1.Details.length > 0) {
                            $.each(item1.Details, (key, item2) => {
                                root.append(` <div class="div-lv4" onclick="SetNode_ParentIdSample('${item2.Node}',${item2.Id},'${item2.Name}','${item2.ObjectId}')"><i class="fa fa-caret-right"></i>${item2.Name}</div>`);
                            });
                        }
                    });
                }
    ...
    function SetNode_ParentIdSample(node, parentId, groupName, phaseGroupId) {
        Global.Data.SampleObj.Node = node;
        Global.Data.SampleObj.ParentId = parentId;
        Global.Data.PhaseGroupId = phaseGroupId;
        Global.Data.TenCumCĐ = groupName;
    ...
    function InitListPhase_Sample() {
        $('#' + Global.Element.JtablePhaseSample).jtable({
            ...
            rowInserted: function (event, data) {
                Global.Data.TotalGiay += data.record.TotalTMU;
                $('#' + Global.Element.JtablePhaseSample).find('.jtable-title-text').html('Danh Sách Công Đoạn của : <span class="red"> ' + Global.Data.TenCumCĐ + '</span> - Tổng thời gian: <span class="blue"> ' + Global.Data.TotalGiay.toFixed(0) + '</span>s');
            },
            fields: {
                Id: {
                    key: true,
                ...
                actions: {
                    ...
                    display: function (data) {
                        ...
                        edit.click(function () {
                            ...
                            $('#phase-code').html(data.record.FullCode);
                            $('#phase-code').attr('code', data.record.Code);
                            ...
                            if (data.record.Video) {
                                ...
                                document.querySelector("video").src = data.record.Video.split('|')[0];
                                // video.load();
                            }
                            else {
                                $('#video-info').html('');
                    ...
                    $.each(_item.Details, (key, item1) => {
                        ...
                        if (item1.Details && item1.Details.length > 0) {
                            $.each(item1.Details, (key, item2) => {
                                ...
                                root.append(`   <div class="div-lv4" >
                                                    <div onclick="BindData('${item2.Id}',${item2.ObjectType})"><i class="fa fa-caret-right"></i>${item2.Name}</div>
                                                    <i data-toggle="modal" data-target="#popup-phase-lib" onclick="SetPhaseGroup(${item2.Id},${item2.ObjectId} )" class="fa fa-book red" title="Lấy công đoạn từ thư viện"></i>
                                                </div>`);
                            });
                        }
                    });
    ...
    function InitListPhase_View() {
        $('#' + Global.Element.JtablePhase).jtable({
            ...
            rowInserted: function (event, data) {
                Global.Data.TotalGiay += data.record.TotalTMU;
                $('#' + Global.Element.JtablePhase).find('.jtable-title-text').html('Danh Sách Công Đoạn của : <span class="red"> ' + Global.Data.TenCumCĐ + '</span> - Tổng thời gian: <span class="blue"> ' + Global.Data.TotalGiay.toFixed(0) + '</span>s');
            },
            fields: {
                Id: {
                    key: true,
                ...
                Index: {
                    ...
                    width: "7%",
                    display: function (data) {
                        var txt = '<span class="red bold">' + data.record.FullCode + '</span>';
                        return txt;
                    }
                },
                Name: {
                    ...
                    display: function (data) {
                        if (data.record.Status == "Soạn thảo") {
                            var txt = $('<input class="form-control"  type="text" value="' + data.record.Name + '" />');
                            txt.keypress(function (e) {
                                var charCode = (e.which) ? e.which : event.keyCode;
                                if (charCode == 13) {
                                    UpdatePhaseName(data.record.Id, txt.val());
                                }
                            });
                ...
                action: {
                    ...
                    display: function (data) {
                        ...
                        var copy = $('<i   title="Copy công đoạn" class="fa fa-files-o clickable blue"  ></i>');
                        copy.click(function () {
                        ...
                        if (data.record.Status == 'Đã duyệt') {
                            var libBut = $('<i data-toggle="modal" data-target="#' + Global.Element.PopupLibrary + '" title="Đưa vào thư viện phân tích" class="fa fa-book clickable blue"  ></i>');
    ...
    function GetPhase(_phaseId, _isSample) {
        $.ajax({
            url: Global.UrlAction.GetPhaseById,
            type: 'POST',
            data: JSON.stringify({ 'phaseId': _phaseId }),
            contentType: 'application/json charset=utf-8',
            beforeSend: function () { $('#loading').show(); },
            success: function (data) {
                $('#loading').hide();
                GlobalCommon.CallbackProcess(data, function () {
                    if (data.Result == "OK") {
                        var phaseObj = data.Records;
                        phaseObj.TotalTMU = Math.round(phaseObj.TotalTMU * 1000) / 1000;
                        $('#workersLevel').val(phaseObj.WorkerLevelId);
                        $('#TotalTMU').html(phaseObj.TotalTMU);
                        $('#phase-Des').val(phaseObj.Description);
                        $('#phaseID').val(phaseObj.Id);
                        $('#phase-name').val(phaseObj.Name).change();
                        $('#phase-code').attr('code', phaseObj.Code);
                        $('#phase-code').html(phaseObj.FullCode);
                        $('#phase-index').val(phaseObj.Index);

                        if (phaseObj.timePrepares.length > 0) {
                            $.each(phaseObj.timePrepares, function (i, item) {
                                Global.Data.TimePrepareArray.push(item);
                            });
                            // ReloadListTimePrepare();
                            $('#time-repare-name').val(`${Global.Data.TimePrepareArray[0].Name} - TMU: ${Global.Data.TimePrepareArray[0].TMUNumber}`);
                        }
                        $('#equipmentId').val(phaseObj.EquipmentId);
                        $('#equipmentName').val(phaseObj.EquipName);
                        $('#equiptypedefaultId').val(phaseObj.EquipTypeDefaultId);
                        $('#E_info').val(phaseObj.EquipDes);
                        $('#ApplyPressure').val(phaseObj.ApplyPressuresId);
                        $('#islibs').prop('checked', phaseObj.IsLibrary);
                        $('#chooseApplyPressure').hide();
                        if (phaseObj.ApplyPressuresId != 0)
                            $('#chooseApplyPressure').show();
                        Global.Data.PhaseManiVerDetailArray.length = 0;

                        if (phaseObj.actions.length > 0) {
                            $.each(phaseObj.actions, function (i, item) {
                                item.OrderIndex = i + 1;
                            });
                            $.each(phaseObj.actions, function (i, item) {
                                var obj = {
                        ...
                        AddEmptyObject();
                        ReloadListMani_Arr();
                        $('[percentequipment]').val(phaseObj.PercentWasteEquipment);
                        $('[percentmanipulation]').val(phaseObj.PercentWasteManipulation);
                        $('[percentdb]').val(phaseObj.PercentWasteSpecial);
                        $('[percentnpl]').val(phaseObj.PercentWasteMaterial);
                        UpdateIntWaste();
                        Global.Data.isInsertPhase = false;
                        Global.Data.Video = phaseObj.Video;
                        // var video = document.getElementsByTagName('video')[0];

                        // var sources = video.getElementsByTagName('source');
                        if (phaseObj.Video) {
                            $('#video-info').html(phaseObj.Video.split('|')[1] + '  <i onclick="removeVideo()" title="Gỡ video" class="fa fa-trash-o red clickable"></i>')

                            // sources[0].src = phaseObj.Video.split('|')[0];
                            //sources[1].src = phaseObj.Video.split('|')[0];
                            document.querySelector("video").src = phaseObj.Video.split('|')[0];
                            // video.load();
                        }
                        else {
                            $('#video-info').html('');
                        ...
                        $('#phase-status').html(phaseObj.Status);
                        $('#phase-status').attr('class', '');
                        let cls = 'normal-text';
                        ...
                        $('[submit-phase]').show();
                        // TODO
                        if (phaseObj.Status == "Chờ duyệt" || phaseObj.Status == "Đã duyệt") {
                            $('[save-phase],[submit-phase]').hide();
                            $('[not-approve-phase]').show();
                        }
                        else
                        ...
                        if (_isSample) {
                            ...
                            if (phaseObj.Status == "Đã duyệt") {
                                InsertViewLog(phaseObj.Id, phaseObj.WorkshopId, ("Mã hàng: " + phaseObj.ProductName + " - Công đoạn: " + phaseObj.Name));
                            }
                        }
                    }
                    else
    ...
    function BindData(Id, oType, wkId) {
        ...
        if (_filter) {
            ...
            if (oType == 6) {
                ...
                Global.Data.TenCumCĐ = _filter.Name;
    ...
    function InitPopupCreateWorkshop() {
        ...
        $('#wk-name').change(() => {
            $('#workshop-Description').val($('#wk-name option:selected').attr('des'));
        });
    ...
    ********************************************************************************************************************/
    ...
    function GetTechProcess(node, parentID) {
        $.ajax({
            ...
            type: 'post',
            data: JSON.stringify({ 'parentId': parentID, 'node': node, 'isGetNull': true }),
            contentType: 'application/json',
            beforeSend: function () { $('#loading').show(); },
            success: function (result) {
                $('#loading').hide();
                    ...
                    if (result.Result == "OK") {
                        ...
                        if (result.Data.Id == 0) {
                            ...
                            if (result.Data.details.length > 0) {
                                ...
                                ResetWorkingBox(0);
                                $('#qtcn_createUser').html('');
                                $('#qtcn_createDate').html('');
                            }
                            else
                            ...
                            $('#pricePerMinute').val(result.Data.PricePerMinute);
                            ...
                            $('#qtcn_createUser').html(result.Data.CreateBy);
                            $('#qtcn_createDate').html(result.Data.CreateAt);
    ...
    function InitPopupExportQTCN() {
        ...
        $('[texch-export1]').click(function () {
            Global.Data.techExportUrl = Global.UrlAction.ExportToExcel_QTCN + "?parentId=" + Global.Data.SampleObj.ParentId + "&fileType=" + 1 + "&isGetNull=true";
            if (Global.Data.AfterSave) {
                window.location.href = Global.Data.techExportUrl;
                Global.Data.AfterSave = false;
            }
        ...
        $('[texch-export2]').click(function () {
            Global.Data.techExportUrl = Global.UrlAction.ExportToExcel_QTCN + "?parentId=" + Global.Data.SampleObj.ParentId + "&fileType=" + 2 + "&isGetNull=true";
            if (Global.Data.AfterSave) {
                window.location.href = Global.Data.techExportUrl;
                Global.Data.AfterSave = false;
            }
        ...
        $('[texch-export3]').click(function () {
            Global.Data.techExportUrl = Global.UrlAction.ExportToExcel_QTCN + "?parentId=" + Global.Data.SampleObj.ParentId + "&fileType=" + 3 + "&isGetNull=true";
            if (Global.Data.AfterSave) {
                window.location.href = Global.Data.techExportUrl;
                Global.Data.AfterSave = false;
            }
        ...
        $('[texch-export4]').click(function () {
            Global.Data.techExportUrl = Global.UrlAction.ExportToExcel_QTCN + "?parentId=" + Global.Data.SampleObj.ParentId + "&fileType=" + 4 + "&isGetNull=true";
            if (Global.Data.AfterSave) {
                window.location.href = Global.Data.techExportUrl;
                Global.Data.AfterSave = false;
            }
        ...
        $('[texch-export6]').click(function () {
            Global.Data.techExportUrl = Global.UrlAction.ExportToExcel_QTCN + "?parentId=" + Global.Data.SampleObj.ParentId + "&fileType=" + 6 + "&isGetNull=true";
            if (Global.Data.AfterSave) {
                window.location.href = Global.Data.techExportUrl;
                Global.Data.AfterSave = false;
            }
        ...
        $('[texch-export7]').click(function () {
            Global.Data.techExportUrl = Global.UrlAction.ExportToExcel_QTCN + "?parentId=" + Global.Data.SampleObj.ParentId + "&fileType=" + 7 + "&isGetNull=true";
            if (Global.Data.AfterSave) {
                window.location.href = Global.Data.techExportUrl;
                Global.Data.AfterSave = false;
            }
        ...
        $('[texch-export8]').click(function () {
            Global.Data.techExportUrl = Global.UrlAction.ExportToExcel_QTCN + "?parentId=" + Global.Data.SampleObj.ParentId + "&fileType=" + 8 + "&isGetNull=true";
            if (Global.Data.AfterSave) {
                window.location.href = Global.Data.techExportUrl;
                Global.Data.AfterSave = false;
            }
        ...
        $('[texch-export5]').click(function () {
            Global.Data.techExportUrl = "/ProAna/ExportToExcel_5?parentId=" + Global.Data.SampleObj.ParentId + "&isGetNull=true";
            if (Global.Data.AfterSave) {
                window.location.href = Global.Data.techExportUrl;
                Global.Data.AfterSave = false;
            }
    ...
    function GetTech(node) {
        ...
        $.ajax({
            url: Global.UrlAction.GetTech,
            type: 'post',
            data: JSON.stringify({ 'parentId': Global.Data.CurrentObj.Id - 1, 'node': Global.Data.CurrentObj.Node, 'isGetNull': true }),
            contentType: 'application/json',
            beforeSend: function () { $('#loading').show(); },
            success: function (result) {
                $('#loading').hide();
    ...
    function UpdatePhaseName(phaseId, newName) {
        $.ajax({
            url: Global.UrlAction.UpdatePhaseName,
            type: 'post',
            data: JSON.stringify({ 'phaseId': phaseId, 'newName': newName }),
            contentType: 'application/json',
            beforeSend: function () { $('#loading').show(); },
            success: function (result) {
                $('#loading').hide();
                    ...
                    display: function (data) {
                        ...
                        txt.change(function () {
                            ...
                            if (Global.Data.ManipulationList.length > 0 && code != '') {
                                ...
                                var flag = false;
                                if (code.length >= 4) {
                                    if ((code.substring(0, 1) == "C" && !isNaN(code.substring(0, code.length - 3))) || code.substring(0, 2) == "SE") {
    ...
    function GetManipulationEquipmentInfoByCode(code, callback) {
        ...
        if (code.indexOf("C") > -1) {
            ...
            if (codeType == "C" && !isNaN(code.substring(0, code.length - 3)))
                isGetTMU = true;
        }
        ...
        if (isGetTMU == true) {
            if ($('#equipmentId').val() == "" || $('#equiptypedefaultId').val() == "") {
                ...
                if (code.substring(0, 1) == "C" && !isNaN(code.substring(0, code.length - 3))) {
    ...
    function GetPhasesById() {
        $.ajax({
            url: Global.UrlAction.GetPhaseSusgestById,
            type: 'POST',
    ...
    function InitPopupLibrary() {
        $('#' + Global.Element.PopupLibrary).on('shown.bs.modal', function () {
            $('div.divParent').attr('currentPoppup', Global.Element.PopupLibrary.toUpperCase());
        });

        $('[save-phase-library]').click(function () {
            let value = $('#phase-group-library').val();
            if (!value || value == '0') {
                GlobalCommon.ShowMessageDialog("Vui lòng chọn cụm công đoạn.", function () { $('#phase-group-library').focus(); }, "Lỗi thao tác");
            }
            else {
                $.ajax({
                    url: Global.UrlAction.MoveToLibrary,
                    type: 'post',
                    data: JSON.stringify({ 'phaseId': $('#phase-group-library-id').val(), 'phaseGroupId': value }),
                    contentType: 'application/json',
                    beforeSend: function () { $('#loading').show(); },
                    success: function (result) {
                        $('#loading').hide();
                        GlobalCommon.CallbackProcess(result, function () {
                            if (result.Result == "OK") {
                                $('#phase-group-library-id').val(0)
                                $('[close-phase-library]').click();
                            }
                            else
                                GlobalCommon.ShowMessageDialog(msg, function () { }, "Đã có lỗi xảy ra trong quá trình xử lý.");
                        }, false, Global.Element.PopupModule, true, true, function () {
        ...
        $('[close-phase-library]').click(function () {
            $('div.divParent').attr('currentPoppup', '');
        });

        $('[re-phase-group-library]').click(() => {
            GetPhaseGroupSelect('phase-group-library')
        });
    }
    ...
    function InitListPhaseLib() {
        $('#' + Global.Element.JtablePhaseLib).jtable({
            ...
            actions: {
                listAction: Global.UrlAction.GetListPhaseLib,
            },
            messages: {
            },
            datas: {
                jtableId: Global.Element.JtablePhaseLib,
            },
            fields: {
                Id: {
                    key: true,
    ...
    function ReloadListPhase_Lib() {
        $('#' + Global.Element.JtablePhaseLib).jtable('load', { 'phaseGroupId': Global.Data.PhaseGroupId });
    }

    function InitPopupPhaseLib() {
        $('#' + Global.Element.PopupPhaseLib).on('shown.bs.modal', function () {
            ReloadListPhase_Lib();
            $('div.divParent').attr('currentPoppup', Global.Element.PopupPhaseLib.toUpperCase());
        });

        $('#choose-phase-lib').click(function () {
            var $selectedRows = $('#' + Global.Element.JtablePhaseLib).jtable('selectedRows');
            if ($selectedRows.length > 0) {
                successCount = 0;
                flag = false;
                ids = [];
                $selectedRows.each(function () {
                    ...
                    flag = false;
                    //push record into array 
                    //$.each(Global.Data.JtablePhaseLib, function (i, item) {
                    //    if (item.TimePrepareId == record.Id) {
                    //        GlobalCommon.ShowMessageDialog("Thời Gian Chuẩn Bị này đã được chọn. Vui lòng kiểm tra lại.!", function () { }, "Thông báo Thời Gian Chuẩn Bị Tồn Tại");
                    //        flag = true;
                    //        return false;
        ...
        $('[close-phase-lib]').click(function () {
            $('div.divParent').attr('currentPoppup', '');
        });
    }

    function ImportFromLib(_ids) {
        $.ajax({
            url: Global.UrlAction.ImportFromLibrary,
            type: 'POST',
            data: JSON.stringify({ 'Id': Global.Data.ObjectId, 'phaseIds': _ids }),
            contentType: 'application/json charset=utf-8',
            beforeSend: function () { $('#loading').show(); },
            success: function (data) {
                $('#loading').hide();
