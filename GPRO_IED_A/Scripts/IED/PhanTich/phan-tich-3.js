...
GPRO.namespace('PhanTich');
GPRO.PhanTich = function () {
    var Global = {
        UrlAction: {
            getProductByCustomer: '/PhanTich/GetProductByProductGroupId',
            ...
            Copy_CommoAna: '/ProAna/Copy_CommoAna'
        },
        Element: {
    ...
    this.Init = function () {
        RegisterEvent();
        GetProGroupSelect('pt-product-type');
    ...
    this.ResetTime_Percent = function (index) { ResetTime_Percent(index); }
    var RegisterEvent = function () {
        $('#pt-product-type').change(function () {
            ResetProAnaTreeView(false);
        })
        ...
        $('#jqxTree').on('select', function (event) {
            ...
            switch (type) {
                ...
                case 1:
                    $('[type="' + (type + 1) + '"]').show();
                    $('[type="' + type + 'a"]').show();
                    if (Global.Data.Copy_CommoAnaId != 0) {
                        $('#1paste').show();
                        $('#jqxMenu').css('height', '140px');
                    }
                    else {
                        $('#1paste').hide();
                ...
                case 6:
                    ...
                    $('[type="' + type + 'a"]').show();
                    $('#paste').hide();
                    if (Global.Data.Copy_CommoAnaPhaseGroupId2 != 0) {
                        $('#6paste').show();
                        $('#jqxMenu').css('height', '140px');
                    }
        ...
        $('#jqxTree').on('expand', function (event) {
            ...
            if (loader) {
                if (selectItem.id != '0') {
                    if (selectItem.value == 11) {
                        //load ma hang
                        GetMaHangsByKhachHang(selectItem.id, selectItem.label, 13, loaderItem);
                    }
                    else {
                        var type = 0;
        ...
        $('[save-workshop]').click(function () {
            if (($('#wk-name option:selected').val() == '' || $('#wk-name option:selected').val() == '0') &&
                ...
                if (!$('.wk-select-box').hasClass('hide')) {
                    ...
                    //thêm mới cả 2
                    Global.Data.ModelProAna.Description = $('#workshop-Description').val();
                    Global.Data.ModelProAna.Name = $('#wk-new-name').val();
                    Global.Data.ModelProAna.ObjectId = 0;
                }
        ...
        $('[save-phasegroup]').click(function () {
            if (($('#phaseGroup-name option:selected').val() == '' || $('#phaseGroup-name option:selected').val() == '0') &&
                ...
                if (!$('.phaseGroup-name-select').hasClass('hide')) {
                    ...
                    //thêm mới cả 2
                    Global.Data.ModelProAna.Description = $('#phaseGroup-Description').val();
                    Global.Data.ModelProAna.Name = $('#phaseGroup-new-name').val();
                    Global.Data.ModelProAna.ObjectId = 0;
                }
        ...
        $('[add_phasegroup-name]').click(function () {
            $('.phaseGroup-name-input').removeClass('hide');
            $('.phaseGroup-name-select').addClass('hide');
        });

        $('[add_wk-name]').click(() => {
            $('.wk-input-box').removeClass('hide');
            $('.wk-select-box').addClass('hide');
        });

        initQTCNControlsEvents();


    }

    function GetProGroupSelect(controlName) {
        $.ajax({
    ...
    /*********************************************** TREEVIEW ***********************************************************/
    function ResetProAnaTreeView(donothing) {
        Global.Data.KhachHangs.length = 0;
        Global.Data.index = 0;
        if (Global.Data.position != 0)
            Global.Data.height = Global.Data.position;
        //clear old item
        ...
        if (!Global.Data.is) {
            GetProductGroups(-10001, null);
            Global.Data.is = true;
        }
        else
            GetProductGroups(-10000, null);
    }

    function GetProductGroups(parentId, loaderItem) {
        $.ajax({
            url: '/productgroup/GetSelectList',
            type: 'POST',
            data: JSON.stringify({ 'proGroupId': $('#pt-product-type').val() }),
            contentType: 'application/json charset=utf-8',
            success: function (data) {
                GlobalCommon.CallbackProcess(data, function () {
                    if (data.Result == "OK") {
                        ...
                        $.each(data.Data, function (i, item) {
                            if (item.Value != 0) {
                                ...
                                //khach hang
                                id = ((-9999) + Global.Data.KhachHangs.length);
                                Global.Data.KhachHangs.push(item);
    ...
    function GetMaHangsByKhachHang(parentId, value, getType, loaderItem) {
        var cust = Global.Data.KhachHangs.filter(x => x.Name.trim() == value.trim())[0];
        if (cust) {
            $.ajax({
                url: Global.UrlAction.getProductByCustomer,
                type: 'POST',
                data: JSON.stringify({ 'progroupId': cust.Value }),
                contentType: 'application/json charset=utf-8',
                        ...
                        if (data.Result == "OK") {
                            ...
                            if (data.Data != null && data.Data.CommoAna != null && data.Data.CommoAna.length > 0) {
                                $.each(data.Data.CommoAna, function (i, item) {
                                    if (Global.Data.useTKC || item.Name != 'Thiết kế chuyền') {
                                        var obj = {
                                            'id': item.Id,
                                            'parentid': parentId,
                                            'text': item.Name,
    ...
    function GetCommoAnaItems(parentId, value, getType, loaderItem) {
        $.ajax({
            ...
            type: 'POST',
            data: JSON.stringify({ 'parentId': parentId, 'value': value, 'Type': getType, 'year': 2020 }),
            contentType: 'application/json charset=utf-8',
            beforeSend: function () { $('#loading').show(); },
            success: function (data) {
                GlobalCommon.CallbackProcess(data, function () {
                    ...
                    display: function (data) {
                        ...
                        text.click(function () {
                            ...
                            if (data.record.Video) {
                                ...
                                $('#video-info').html(data.record.Video.split('|')[1])
    ...
    function ReloadListTimePrepare_Chooise() {
        $('#' + Global.Element.jtable_Timeprepare_Chooise).jtable('load', { 'keyword': $('#keyword-time').val() });
    }
                    ...
                    display: function (data) {
                        /*      var txt = $('<input class="form-control" des type="text" value="' + data.record.ManipulationName + '" />');
                         ...
                         txt.keypress(function (e) {
                          ...
                          txt.click(function () { txt.select(); })
                         return txt; */
                        return data.record.ManipulationName;
                    }
    ...
    function GetTechProcess(node, parentID) {
        $.ajax({
            ...
            type: 'post',
            data: JSON.stringify({ 'parentId': parentID, 'node': node, 'isGetNull': false }),
            contentType: 'application/json',
            beforeSend: function () { $('#loading').show(); },
            success: function (result) {
                $('#loading').hide();
                    ...
                    if (result.Result == "OK") {
                        ...
                        if (result.Data.Id == 0) {
                            ...
                            */
                            ...
                            $('#' + Global.Element.JtableTech_Cycle).find('tbody').empty().append('<tr><td colspan="9">Không có dữ liệu.</td></tr>'); 
                            GlobalCommon.ShowMessageDialog('Mã hàng chưa có quy trình công nghệ!.', function () { }, "Lỗi thao tác");
                            ResetWorkingBox(0);
    ...
    function initQTCNControlsEvents() {
        //#region ******************************* Tech process *****************************************************/
            ...
            success: function (result) {
                ...
                GlobalCommon.CallbackProcess(result, function () {
                    if (result.Result == "OK") {
                        ...
                        $('#phaseGroup-name,#phaseGroup-new-name,#phaseGroup-Description,#caDescription,#wk-new-name').val('');

                        $('.phaseGroup-name-select').removeClass('hide');
                        $('.phaseGroup-name-input').addClass('hide');
                    }
                    else
                        GlobalCommon.ShowMessageDialog(msg, function () { }, "Đã có lỗi xảy ra trong quá trình xử lý.");
                }, false, Global.Element.PopupModule, true, true, function () {
    ...
    $("#jqxMenu").on('itemclick', function (event) {
        ...
        switch (item) {
            ...
            case "Sao chép cụm công đoạn":
                ...
                break;
            case "Sao chép nội dung cụm công đoạn":
                $("#jqxMenu").jqxMenu('close');
                Global.Data.Copy_CommoAnaPhaseGroupId2 = selectedItem.id;
                break;
            case "Dán cụm công đoạn":
                $("#jqxMenu").jqxMenu('close');
                Global.Data.ParentID = selectedItem.id;
                Copy_CommoAnaPhaseGroup(true);
                break;
            case "Dán nội dung cụm công đoạn":
                $("#jqxMenu").jqxMenu('close');
                Global.Data.ParentID = selectedItem.id;
                Copy_CommoAnaPhaseGroup(false);
                break;
            case "Sao chép nội dung phân tích":
                $("#jqxMenu").jqxMenu('close');
                Global.Data.Copy_CommoAnaId = parseInt(selectedItem.id);
                break;
            case "Dán nội dung phân tích":
                $("#jqxMenu").jqxMenu('close');
                $('#1paste').hide();
                Global.Data.ParentID = selectedItem.id;
                Copy_CommoAna();
    ...
    function Copy_CommoAnaPhaseGroup(copyFull) {
        var _copyObjectId = copyFull ? Global.Data.Copy_CommoAnaPhaseGroupId : Global.Data.Copy_CommoAnaPhaseGroupId2;
        $.ajax({
            url: Global.UrlAction.Copy_CommoAnaPhaseGroup,
            type: 'POST',
            data: JSON.stringify({ 'CopyObjectId': _copyObjectId, 'ObjectId': Global.Data.ParentID, 'copyFull': copyFull }),
            contentType: 'application/json charset=utf-8',
            beforeSend: function () { $('#loading').show(); },
            success: function (data) {
                $('#loading').hide();
    ...
    function Copy_CommoAna() {
        $.ajax({
            url: Global.UrlAction.Copy_CommoAna,
            type: 'POST',
            data: JSON.stringify({ 'CopyObjectId': Global.Data.Copy_CommoAnaId, 'ObjectId': Global.Data.ParentID }),
            contentType: 'application/json charset=utf-8',
            beforeSend: function () { $('#loading').show(); },
            success: function (data) {
                $('#loading').hide();
    ...
    function InitPopupExportQTCN() {
        ...
        $('[texch-export1]').click(function () {
            Global.Data.techExportUrl = Global.UrlAction.ExportToExcel + "?parentId=" + Global.Data.ParentID + "&fileType=" + 1 +"&isGetNull=false";
            window.location.href = Global.Data.techExportUrl;
            //if (Global.Data.AfterSave) {
            //    window.location.href = Global.Data.techExportUrl;
            //    Global.Data.AfterSave = false;
        ...
        $('[texch-export2]').click(function () {
            Global.Data.techExportUrl = Global.UrlAction.ExportToExcel + "?parentId=" + Global.Data.ParentID + "&fileType=" + 2 + "&isGetNull=false";
            window.location.href = Global.Data.techExportUrl;
            //if (Global.Data.AfterSave) {
            //    window.location.href = Global.Data.techExportUrl;
            //    Global.Data.AfterSave = false;
        ...
        $('[texch-export3]').click(function () {
            Global.Data.techExportUrl = Global.UrlAction.ExportToExcel + "?parentId=" + Global.Data.ParentID + "&fileType=" + 3 + "&isGetNull=false";
            window.location.href = Global.Data.techExportUrl;
            //if (Global.Data.AfterSave) {
            //    window.location.href = Global.Data.techExportUrl;
            //    Global.Data.AfterSave = false;
        ...
        $('[texch-export4]').click(function () {
            Global.Data.techExportUrl = Global.UrlAction.ExportToExcel + "?parentId=" + Global.Data.ParentID + "&fileType=" + 4 + "&isGetNull=false";
            window.location.href = Global.Data.techExportUrl;
            //if (Global.Data.AfterSave) {
            //    window.location.href = Global.Data.techExportUrl;
            //    Global.Data.AfterSave = false;
