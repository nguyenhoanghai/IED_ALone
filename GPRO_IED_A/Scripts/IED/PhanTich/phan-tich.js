...
GPRO.PhanTich = function () {
    var Global = {
        UrlAction: {
            getProductByCustomer: '/PhanTich/GetProductByCustomerId',
    ...
    function ResetProAnaTreeView(donothing) {
        ...
        if (!Global.Data.is) {
            GetCustomers(-10001, null);
            Global.Data.is = true;
        }
        else
            GetCustomers(-10000, null);
    }

    function GetCustomers(parentId, loaderItem) {
        $.ajax({
    ...
    function GetMaHangsByKhachHang(parentId, value, getType, loaderItem) {
        ...
        if (cust) {
            $.ajax({
                ...
                type: 'POST',
                data: JSON.stringify({ 'customerId': cust.Value }),
                contentType: 'application/json charset=utf-8',
