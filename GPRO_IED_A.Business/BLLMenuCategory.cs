...
namespace GPRO_IED_A.Business
{
    public class BLLMenuCategory
    {
        ...
        private SCategory CheckMenuName(int Id, string menuCategoryName, int companyId, IEDEntities db)
        {
            return (from x in db.SCategories
        ...
        public List<CategoryModel> GetCategorysByUserId(int userId, int companyId, string sorting, string position)
        {
            using (var db = new IEDEntities())
            {
                ...
                try
                {
                    ...
                    if (listPermissionUrl != null && listPermissionUrl.Count > 0)
                    {
                        categories = (from x in db.SCategories
        ...
        public List<CategoryModel> GetCategorysByUserId(int userId, int companyId, string sorting, string position, string keyword, string searchBy, string moduleId)
        {
            using (var db = new IEDEntities())
            {
                ...
                try
                {
                    ...
                    if (listPermissionUrl != null && listPermissionUrl.Count > 0)
                    {
                        cateMenus = (from x in db.SCategories
                                     where ((x.CompanyId == null && listPermissionUrl.Contains(x.Link)) || (x.CompanyId == companyId)) && !x.IsDeleted
                                     orderby (sorting)
                                     select x);
        ...
        private List<CategoryModel> SearchByModule(int userId, int companyId, string sorting, string moduleId, IEDEntities db)
        {
        ...
        private List<CategoryModel> SearchByPosition(int userId, int companyId, string sorting, string position, IEDEntities db)
        {
            List<CategoryModel> categories = null;
            try
            {
        ...
        private List<CategoryModel> SearchByKeyword(int userId, int companyId, string sorting, string keyword, IEDEntities db)
        {
            List<CategoryModel> categories = null;
            try
            {
        ...
        public List<CategoryModel> GetCategoriesByCompanyIdNotSystem(int companyId)
        {
            using (var db = new IEDEntities())
            {
                ...
                try
                {
                    var categories = (from x in db.SCategories
        ...
        private IQueryable<CategoryModel> GetCategorysByModuleId(int moduleId, string position, int companyId, IEDEntities db)
        {
        ...
        public List<ModelSelectItem> GetMenuCategoriesByModuleId(int id, int companyId)
        {
            using (var db = new IEDEntities())
            {
                return (from x in db.SCategories where !x.IsDeleted && x.ModuleId == id && (x.CompanyId == companyId || !x.CompanyId.HasValue) select new ModelSelectItem() { Value =x.Id, Name = x.Name }).ToList();
            }
        }
    }
}
