...
namespace GPRO_IED_A.Business
{
    public class BLLRole
    {
        ...
        public SRoLe CheckName(int Id, string Name, int companyId, IEDEntities db)
        {
            return (from x in db.SRoLes where !x.IsDeleted && x.CompanyId == companyId && x.Id != Id && x.RoleName.Trim().ToUpper().Equals(Name.Trim().ToUpper()) select x).FirstOrDefault();
        }
        ...
        public List<ModelRolePermission> GetListRolePermissionByRoleId(int roleId)
        {
            using (var db = new IEDEntities())
            {
                return (from x in db.SRolePermissions
