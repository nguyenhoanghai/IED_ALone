...
namespace GPRO_IED_A.Business
{
    public class BLLLabourDivision
    {
        ...
        public PagedList<LabourDivisionModel> GetList_Version(int labourId, int startIndexRecord, int pageSize, string sorting)
        {
            try
            {
                using (db = new IEDEntities())
            ...
            try
            {
                using (db = new IEDEntities())
                {
                    ...
                    else
                    {
                        ...
                        {
                            labourObj.T_LabourDevision_Ver = new Collection<T_LabourDevision_Ver>();
                            var labourVer = new T_LabourDevision_Ver();
                            labourVer.CreatedDate = labourObj.CreatedDate;
                            labourVer.CreatedUser = labourObj.CreatedUser;
                            labourVer.TotalPosition = objModel.TotalPosition;
                            ...
                            for (int i = 0; i < objModel.Positions.Count; i++)
                            {
                                ...
                                labourVer.T_LinePosition.Add(linePo);
                            }
                            labourObj.T_LabourDevision_Ver.Add(labourVer);
                        }
        ...
        public ResponseBase Update(LabourDivisionModel objModel)
        {
            try
            {
                using (db = new IEDEntities())
                {
                    ...
                    if (labourObj != null)
                    {
                        ...
                        var labourVer = new T_LabourDevision_Ver();
                        labourVer.CreatedDate = labourObj.UpdatedDate.Value;
                        labourVer.CreatedUser = objModel.ActionUser;
                        labourVer.TotalPosition = objModel.TotalPosition;
                        labourVer.T_LinePosition = new List<T_LinePosition>();
                        labourVer.LabourDivisionId = labourObj.Id;
                        ...
                        db.T_LabourDevision_Ver.Add(labourVer);
                        rs.IsSuccess = true;
                    }
                    else
                    {
        ...
        public ResponseBase UpdateCover(LabourDivisionModel objModel)
        {
            try
            {
                using (db = new IEDEntities())
                {
                    ...
                    if (labourObj != null)
                    {
                        ...
                        var labourVer = db.T_LabourDevision_Ver.FirstOrDefault(x => !x.IsDeleted && x.Id == objModel.LabourDivisionVerId);
                        if (labourVer != null)
                        {
                            labourVer.CreatedDate = labourObj.UpdatedDate.Value;
                            labourVer.CreatedUser = objModel.ActionUser;
                            ...
                            // delete old
                            var oldLinePos = db.T_LinePosition.Where(x => x.LabourDivisionVerId == objModel.LabourDivisionVerId);
                            if (oldLinePos != null && oldLinePos.Count() > 0)
                            {
                                var _ids = oldLinePos.Select(x => x.Id).ToList();
                                var childs = db.T_LinePositionDetail.Where(x => _ids.Contains(x.Line_PositionId)).ToList();
                                foreach (var _item in childs)
                                {
                                    db.T_LinePositionDetail.Remove(_item);
                                }

                                foreach (var _item in oldLinePos)
                                {
                                    db.T_LinePosition.Remove(_item);
                                }
                            }
                            ...
                            if (objModel.Positions != null && objModel.Positions.Count > 0)
                            {
                                for (int i = 0; i < objModel.Positions.Count; i++)
                                {
                                    ...
                                    linePo.CreatedDate = DateTime.Now;
                                    linePo.OrderIndex = i + 1;
                                    linePo.LabourDivisionVerId = labourVer.Id;
        ...
        {
            return db.T_LabourDevision_Ver.FirstOrDefault(x => !x.IsDeleted && x.Id == Id);
        }
        ...
        public LabourDivisionModel GetById(int labourVerId)
        {
            ...
            try
            {
                using (db = new IEDEntities())
                {
                    var obj = GetVersion(labourVerId);
                    if (obj != null)
                    {
                        ...
                        objReturn.LabourDivisionVerId = obj.Id;

                        #region positions
                        var linPos = db.T_LinePosition.Where(x => !x.IsDeleted && x.LabourDivisionVerId == labourVerId).Select(x => new LinePositionModel()
                        {
                            Id = x.Id,
        ...
        public LinePositionsAndTechProcessActiveVersionModel GetLinePositionById(int labourVerId)
        {
            try
            {
                using (db = new IEDEntities())
                {
                    ...
                    var linPos = db.T_LinePosition.Where(x => !x.IsDeleted && x.LabourDivisionVerId == labourVerId).Select(x => new LinePositionModel()
                    {
                        ...
                        IsHasBTP = x.IsHasBTP,
                        IsHasExitLine = x.IsHasExitLine,
                        TechProVerId = x.T_LabourDevision_Ver.T_LabourDivision.TechProVer_Id,
                    }).ToList();
        ...
        public LabourDivisionModel RefreshById(int labourVerId)
        {
            var objReturn = new LabourDivisionModel();
            try
            {
                using (db = new IEDEntities())
                {
                    ...
                    if (obj != null)
                    {
                        var _qtcn = db.T_TechProcessVersion.FirstOrDefault(x => x.Id == obj.T_LabourDivision.TechProVer_Id);
                        ...
                        obj.T_LabourDivision.TechProVer_Id = newQTCN.Id;
                        db.SaveChanges();
                    }
                }
            }
        ...
        public List<ModelSelectItem> GetLineModelSelectItems(int parentId)
        {
            ...
            try
            {
                ...
                {
                    return db.T_LabourDivision
                        .Where(x => !x.IsDeleted && x.ParentId == parentId)
                        .Select(x => new ModelSelectItem() { Value = x.Id, Name = x.T_Line.Name, Data = x.LineId })
                        .ToList();
                }
            }
            catch (Exception ex) { }
            ...
            try
            {
                using (db = new IEDEntities())
                {
                    ...
                    if (obj != null)
                    {
                        ...
                        #region positions
                        var linPos = db.T_LinePosition.Where(x => !x.IsDeleted && x.LabourDivisionVerId == obj.Id).Select(x => new LinePositionModel()
                        {
                            Id = x.Id,
                            LabourDivisionId = x.LabourDivisionVerId,
                            OrderIndex = x.OrderIndex,
        ...
        public List<ModelSelectItem> GetEmployeeInTKC(int laDe_VersionId)
        {
            using (var db = new IEDEntities())
            {
