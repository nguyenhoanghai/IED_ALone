...
namespace GPRO_IED_A.Business
{
    public class BLLMenu
    {
        ...
        private SMenu CheckMenuName(string menuName, int? menuId, IEDEntities db)
        {
            if (menuId == null)
            {
                return (from x in db.SMenus where !x.IsDeleted && x.MenuName.Trim().ToUpper().Equals(menuName.Trim().ToUpper()) select x).FirstOrDefault();
            }
            else
            {
                return (from x in db.SMenus where !x.IsDeleted && x.Id != menuId && x.MenuName.Trim().ToUpper().Equals(menuName.Trim().ToUpper()) select x).FirstOrDefault();
            }
        }
                ...
                using (var db = new IEDEntities())
                {
                    switch (checkPermissionType)
                    {
                        case (int)eCheckPermissionType.CheckPermissionByModule:
                            {
                                listModelMenu = (from x in db.SMenus
